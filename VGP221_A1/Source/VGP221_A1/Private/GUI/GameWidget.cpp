...
void UGameWidget::SetTimeText(int amount)
{
	if (!CurrentTime) return;

	UIScore = amount;
	CurrentTime->SetText(FText::FromString("Score: " + FString::FromInt(UIScore)));
}

void UGameWidget::SetBestText(int amount)
{
	if (!BestTime) return;

	UIScore = amount;
	BestTime->SetText(FText::FromString("Score: " + FString::FromInt(UIScore)));
}
